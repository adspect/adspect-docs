...
[**Adspect**](https://www.adspect.io/) is a powerful and versatile cloud-based service designed to protect ad campaigns--such as
affiliate offers, landing pages, file downloads, and mobile app content--from unwanted traffic.  By "unwanted traffic," we mean:
...
* competitors;
* and other kinds of suspicious or hostile visitors.

It works as follows: visitors (traffic of any kind--ads, e-mail, organic, etc.) have their intrinsic attributes collected
and evaluated by our special PHP script before reaching their final destination.  Adspect performs more than a hundred checks
on each click and determines whether a visitor is relevant or unwanted.  Relevant visitors are directed to your actual
money-making content (the so-called "money page") while unwanted visitors are shown an alternative version of the content that
does not expose any sensitive information (the so-called "safe page.")

:::{note}
You can find quick answers to frequently asked questions in the [Adspect FAQ](https://www.adspect.io/faq).
:::
...
You can get information about your PHP setup using the following PHP script:
...
2. Choose an appropriate integration method and follow instructions on the stream's integration page;
3. Set the stream to the All Money mode and test it to make sure that the money page is displayed correctly;
4. Set the stream to the All Safe mode and test it to make sure that the safe page is displayed correctly;
5. Set the stream to the Filtering mode and test it to make sure that **you get forwarded to the money page** without any errors
   (temporarily disable any manual filters that may block you);
